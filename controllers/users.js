...
async function cleanupOldAvatar(avatar) {
  if (!avatar?.url && !avatar?.filename) return;

  const isCloudinaryAsset =
    typeof avatar.url === "string" &&
    avatar.url.includes("res.cloudinary.com") &&
    typeof avatar.filename === "string" &&
    avatar.filename.includes("/");

  if (isCloudinaryAsset) {
    try {
      await cloudinary.uploader.destroy(avatar.filename);
    } catch (err) {
      console.warn("Failed to delete previous avatar from Cloudinary:", err.message);
    }
    return;
  }

  if (typeof avatar.url === "string" && avatar.url.startsWith("/uploads/")) {
    const fileName = decodeURIComponent(path.basename(avatar.url));
...
module.exports.signup = async (req, res) => {
  ...
  try {
    ...
    const emailResult = await sendEmail(
      email,
      "Wanderlust OTP Verification Code",
      html
    );

    if (emailResult?.mock && process.env.NODE_ENV !== "production") {
      req.flash(
        "success",
        `SMTP not configured. Dev OTP for ${email}: ${otp}`
      );
    } else {
...
module.exports.resendOTP = async (req, res) => {
  ...
  const emailResult = await sendEmail(user.email, "Wanderlust - New OTP", html);

  if (emailResult?.mock && process.env.NODE_ENV !== "production") {
    req.flash("success", `SMTP not configured. Dev OTP: ${otp}`);
  } else {
...
module.exports.sendResetOTP = async (req, res) => {
  ...
  const emailResult = await sendEmail(
    user.email,
    "Wanderlust - Password Reset OTP",
    html
  );

  if (emailResult?.mock && process.env.NODE_ENV !== "production") {
    req.flash("success", `SMTP not configured. Dev reset OTP: ${otp}`);
  } else {
...
module.exports.updateProfile = async (req, res) => {
  ...
  if (req.file) {
    ...
    user.avatar = normalizeUploadedImage(req.file);
  }

  await user.save();

  // ðŸ”¥ STEP 1: Refresh passport session
  req.login(user, (err) => {
    if (err) {
    ...
    // ðŸ”¥ STEP 2: Update locals manually (IMPORTANT)
